...
public class GridGameVisualizer {
	...
	public static Visualizer getVisualizer(int maxX, int maxY){
		...
		v.addObjectClassPainter(ExpGridGame2.CLASSGOAL, new CellPainter(maxX, maxY, goalColors, 0));
		v.addObjectClassPainter(ExpGridGame2.CLASSAGENT, new CellPainter(maxX, maxY, agentColors, 1));
		//v.addObjectClassPainter(ExpGridGame2.CLASSAGENT, new CellPainter(maxX, maxY, ballColors, 1));
		v.addObjectClassPainter(ExpGridGame2.CLASSDIMVWALL, new WallPainter(maxX, maxY, true));
		v.addObjectClassPainter(ExpGridGame2.CLASSDIMHWALL, new WallPainter(maxX, maxY, false));
	...
	static class CellPainter implements ObjectPainter{
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			boolean ballFlag1 = false; boolean ballFlag = false;
			int colInd = 0;
			if(ob.getClassName().equals(ExpGridGame2.CLASSAGENT)){
				colInd = ob.getIntValForAttribute(ExpGridGame2.ATTPN);
				ballFlag1 = ob.getBooleanValForAttribute(ExpGridGame2.ATTBALLOWNER);
				
			}
			else if(ob.getClassName().equals(ExpGridGame2.CLASSGOAL)){
				colInd = ob.getIntValForAttribute(ExpGridGame2.ATTGT);
			}
			
			/*else if(ob.getClassName().equals(ExpGridGame2.CLASSBALLOWNER)){
				System.out.println("Inside class ball owner");
				float rx1 = ob.getIntValForAttribute(ExpGridGame2.ATTX);
				float ry1 = ob.getIntValForAttribute(ExpGridGame2.ATTY);
				System.out.println("x value" +rx1);
				System.out.println("y value" +ry1);
				float bo1 = ob.getIntValForAttribute(ExpGridGame2.ATTBALLOWNER);
				System.out.println("Ball owner is" +bo1);
				colInd = 0;
				ballFlag = true;
			} */
			...
			float rx = ob.getIntValForAttribute(ExpGridGame2.ATTX)*width;
			float ry = cHeight - height - ob.getIntValForAttribute(ExpGridGame2.ATTY)*height;
			...
			//Agent with no ball
			else if(shape == 1 && ballFlag1 == false){
				g2.fill(new Ellipse2D.Float(rx, ry, width, height));
			}
			
			
			else if(shape == 1 && ballFlag1 == true){
				
				//First fill the agent and then the ball val
				g2.fill(new Ellipse2D.Float(rx, ry, width, height));
	...
	static class WallPainter implements ObjectPainter{
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			int wp = ob.getIntValForAttribute(ExpGridGame2.ATTP);
			int e1 = ob.getIntValForAttribute(ExpGridGame2.ATTE1);
			int e2 = ob.getIntValForAttribute(ExpGridGame2.ATTE2);
			...
			int wt = ob.getIntValForAttribute(ExpGridGame2.ATTWT);
			if(wt == 0){
