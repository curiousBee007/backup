 ...
 *
 */
public class GridGameOriginalMechanics extends JointActionModel {
	 ...
	 */
	public GridGameOriginalMechanics(Domain d){
	 ...
	 */
	public GridGameOriginalMechanics(Domain d, double semiWallPassThroughProb){
		rand = RandomFactory.getMapped(0);
		domain = d;
	...
	public List<TransitionProbability> transitionProbsFor(State s, JointAction ja) {
		...
		List <Location2> previousLocations = new ArrayList<GridGameOriginalMechanics.Location2>();
		List <Location2> noopLocations = new ArrayList<GridGameOriginalMechanics.Location2>();
		
		for(GroundedSGAgentAction gsa : gsas){
			Location2 loc = this.getLocation(s, gsa.actingAgent);
			previousLocations.add(loc);
			if(gsa.action.actionName.equals(GridGameOriginal.ACTIONNOOP)){
		...
		for(LocationSetProb sp : outcomeSets){
			...
			for(LocationSetProb csp : cOutcomeSets){
				...
				for(int i = 0; i < csp.locs.size(); i++){
					...
					agent.setValue(GridGameOriginal.ATTX, loc.x);
					agent.setValue(GridGameOriginal.ATTY, loc.y);
				}
	...
	protected State actionHelper(State s, JointAction ja) {
		...
		List <Location2> basicMoveResults = new ArrayList<GridGameOriginalMechanics.Location2>();
		for(int i = 0; i < ja.size(); i++){
			Location2 loc = previousLocations.get(i);
			GroundedSGAgentAction gsa = gsas.get(i);
	 ...
	 * @return a {@link GridGameOriginalMechanics.Location2} object containing the agents position in the world.
	 */
	protected Location2 getLocation(State s, String agentName){
		
		ObjectInstance a = s.getObject(agentName);
		Location2 loc = new Location2(a.getIntValForAttribute(GridGameOriginal.ATTX), a.getIntValForAttribute(GridGameOriginal.ATTY));
	...
	protected Location2 attemptedDelta(String actionName){
		...
		if(actionName.equals(GridGameOriginal.ACTIONNORTH)){
			return new Location2(0, 1);
		}
		else if(actionName.equals(GridGameOriginal.ACTIONSOUTH)){
			return new Location2(0, -1);
		}
		else if(actionName.equals(GridGameOriginal.ACTIONEAST)){
			return new Location2(1, 0);
		}
		else if(actionName.equals(GridGameOriginal.ACTIONWEST)){
			return new Location2(-1, 0);
		}
		else if(actionName.equals(GridGameOriginal.ACTIONNOOP)){
	...
	protected List<Location2> resolvePositionSwaps(List <Location2> originalPositions, List<Location2> desiredPositions){
		...
		List<Location2> resolvedPositions = new ArrayList<GridGameOriginalMechanics.Location2>(desiredPositions);
		List <Location2> newNoopPositions = new ArrayList<GridGameOriginalMechanics.Location2>();
	...
	List <Location2> backupNoOps(List <Location2> originalPositions, List<Location2> desiredPositions, List <Location2> noops){
		...
		List <Location2> resolved = new ArrayList<GridGameOriginalMechanics.Location2>(desiredPositions);
	...
	protected List<Location2> resolveCollisions(List<Location2> originalPositions, List <Location2> desiredPositions){
		...
		List <Location2> finalPoses = new ArrayList<GridGameOriginalMechanics.Location2>();
	...
	protected List <LocationSetProb> getPossibleCollisionOutcomes(List<Location2> originalPositions, List <Location2> desiredPositions){
		...
		if(collissionSets.size() == 0){
			...
			List <LocationSetProb> outcomes = new ArrayList<GridGameOriginalMechanics.LocationSetProb>(1);
			outcomes.add(new LocationSetProb(new ArrayList<GridGameOriginalMechanics.Location2>(desiredPositions), 1.));
			return outcomes;
			
		}
		
		List <LocationSetProb> allOutcomes = new ArrayList<GridGameOriginalMechanics.LocationSetProb>();
	...
	protected Location2 sampleBasicMovement(State s, Location2 p0, Location2 delta, List <Location2> agentNoOpLocs){
		...
		if(delta.x != 0 && !reset){
			reset = this.sampleWallCollision(p0, delta, s.getObjectsOfClass(GridGameOriginal.CLASSDIMVWALL), true);
		}
		
		if(delta.y != 0 && !reset){
			reset = this.sampleWallCollision(p0, delta, s.getObjectsOfClass(GridGameOriginal.CLASSDIMHWALL), false);
		}
	...
	protected List <Location2Prob> getPossibleLocationsFromWallCollisions(State s, Location2 p0, Location2 delta, List <Location2> agentNoOpLocs){
		...
		List <Location2Prob> locs = new ArrayList<GridGameOriginalMechanics.Location2Prob>(2);
		...
		if(delta.x != 0){
			int wc = this.wallCollision(p0, delta, s.getObjectsOfClass(GridGameOriginal.CLASSDIMVWALL), true);
			if(wc == 0){
			...
			int wc = this.wallCollision(p0, delta, s.getObjectsOfClass(GridGameOriginal.CLASSDIMHWALL), false);
			if(wc == 0){
				locs.add(new Location2Prob(p1, 1.)); //agent freely moves
			}
			else if(wc == 1){
	...
	protected boolean sampleWallCollision(Location2 p0, Location2 delta, List <ObjectInstance> walls, boolean vertical){
		...
		for(int i = 0; i < walls.size(); i++){
			...
			if(this.crossesWall(p0, delta, w, vertical)){
				int wt = w.getIntValForAttribute(GridGameOriginal.ATTWT);
				if(wt == 0){ //solid wall
					return true;
				}
	...
	protected boolean crossesWall(Location2 p0, Location2 delta, ObjectInstance w, boolean vertical){
		...
		int wp = w.getIntValForAttribute(GridGameOriginal.ATTP);
		int we1 = w.getIntValForAttribute(GridGameOriginal.ATTE1);
		int we2 = w.getIntValForAttribute(GridGameOriginal.ATTE2);
	...
	protected List<LocationSetProb> getAllLocationSets(List<List<Location2Prob>> locOutcomes){
		...
		List <LocationSetProb> sets = new ArrayList<GridGameOriginalMechanics.LocationSetProb>();
	...
	protected void generateAllLocationSetsHelper(List<List<Location2Prob>> locOutcomes, int i, Location2 [] locArray, double stackedProb, List<LocationSetProb> discovered){
		...
		if(i == locOutcomes.size()){
			//bundle them up
			List <Location2> locs = new ArrayList<GridGameOriginalMechanics.Location2>(locArray.length);
	...
	protected boolean agentsEqual(State s1, State s2){
		...
		List<ObjectInstance> agents1 = s1.getObjectsOfClass(GridGameOriginal.CLASSAGENT);
		for(ObjectInstance a1 : agents1){
			ObjectInstance a2 = s2.getObject(a1.getName());
			
			int x1 = a1.getIntValForAttribute(GridGameOriginal.ATTX);
			int x2 = a2.getIntValForAttribute(GridGameOriginal.ATTX);
			
			int y1 = a1.getIntValForAttribute(GridGameOriginal.ATTY);
			int y2 = a2.getIntValForAttribute(GridGameOriginal.ATTY);
