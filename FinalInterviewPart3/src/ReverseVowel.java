...
public class ReverseVowel {
	...
	public static  HashMap<Integer,Integer> findInteger(String s, int i){
		...
		while(t != 43 && t != 45 && t != 47 && t != 42 && t != 32 && i != s.length()){
		...
		while (!tempStack.isEmpty()){
			int unitVal = tempStack.pop();
			unitVal = unitVal - 48;  
			
			//System.out.println(unitVal);
			sum = (int) (sum +  unitVal* Math.pow(10,power));
			power ++;
			count++;
	...
	public int calculate(String s) {
    	...
    	if((int)t >=48 && (int)t <= 57){
    		...
    		if(i <= s.length() - 2){
    			...
    			if (i <= s.length() - 2 && nextChar >= 48 && nextChar <=57){
    				HashMap<Integer,Integer> hashMap= findInteger(s, i);
    				Iterator iterator = hashMap.keySet().iterator();
    				
    				while (iterator.hasNext()) {
    					   Integer key = (Integer) iterator.next();
    					   Integer value = hashMap.get(key);
    					   operand[countOperand] = key;
    					   countOperand ++;
    					   }
	...
	public static void main(String[] args){
		...
		String str1 = "123";
		HashMap<Integer,Integer> hashMap= findInteger(str1, 0);
